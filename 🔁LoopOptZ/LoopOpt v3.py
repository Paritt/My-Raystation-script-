...
# Last ROW
#############################
lbRoud = Label(root, text= 'Round')
lbRoud.config(width=27, height =2)
lbRoud.grid(column=6 ,row=7)

Roud = StringVar()
textRoud = Entry(root, textvariable = Roud)
textRoud.grid(column=7, row=7)
textRoud.config(width=27)
textRoud.insert(0, "1")
...
def create_contour_and_constrain(roud, roi, cplus, plus,plus_prio,pcon, cminus, minus,minus_prio,mcon,i,dose,plan,po):
    name_plus = f"z{roud}{roi}_+{plus}_{i}"
    name_minus = f"z{roud}{roi}_-{minus}_{i}"
...
def run():
    ...
    for i in range(0,int(NumL.get())):
        ...
        print(f'Loop: {i}')
        create_contour_and_constrain(Roud.get(), ROI1.get(), float(ROI1_convertp.get()), float(ROI1_pdose.get()),float(ROI1_pprio.get()),float(ROI1_pcrop.get())
                                    , float(ROI1_convertm.get()), float(ROI1_mdose.get()),float(ROI1_mprio.get()),float(ROI1_mcrop.get()),int(i),dose,plan,Po)
        if roi2 != "--None--":
             create_contour_and_constrain(Roud.get(), ROI2.get(), float(ROI2_convertp.get()), float(ROI2_pdose.get()),float(ROI2_pprio.get()),float(ROI2_pcrop.get())
                                    , float(ROI2_convertm.get()), float(ROI2_mdose.get()),float(ROI2_mprio.get()),float(ROI2_mcrop.get()),int(i),dose,plan,Po)
        if roi3 != "--None--":
             create_contour_and_constrain(Roud.get(), ROI3.get(), float(ROI3_convertp.get()), float(ROI3_pdose.get()),float(ROI3_pprio.get()),float(ROI3_pcrop.get())
                                    , float(ROI3_convertm.get()), float(ROI3_mdose.get()),float(ROI3_mprio.get()),float(ROI3_mcrop.get()),int(i),dose,plan,Po) 
        if roi4 != "--None--":
             create_contour_and_constrain(Roud.get(), ROI4.get(), float(ROI4_convertp.get()), float(ROI4_pdose.get()),float(ROI4_pprio.get()),float(ROI4_pcrop.get())
