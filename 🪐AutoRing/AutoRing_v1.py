...
def Ring(Roi1,dis,virname,color):
    list_roi = [r.Name for r in patient.Cases[0].PatientModel.RegionsOfInterest]
    ...
    zVir.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [Roi1], 
                                            'MarginSettings': { 'Type': "Expand", 'Superior': str(float(dis)+1), 'Inferior': str(float(dis)+1), 
                                                                'Anterior': str(float(dis)+1), 'Posterior': str(float(dis)+1), 
                                                                'Right': str(float(dis)+1), 'Left': str(float(dis)+1) } }, 
    ...
    if virname in list_roi:
        case.PatientModel.RegionsOfInterest[virname].CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ...
                                ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [Roi2], 
                                            'MarginSettings': { 'Type': "Expand", 'Superior': str(float(dis)+float(dis)+1), 'Inferior': str(float(dis)+float(dis)+1), 
                                                                'Anterior': str(float(dis)+float(dis)+1), 'Posterior': str(float(dis)+float(dis)+1), 
                                                                'Right': str(float(dis)+float(dis)+1), 'Left': str(float(dis)+float(dis)+1) } }, 
                                ResultOperation="Subtraction", 
                                ResultMarginSettings={ 'Type': "Expand", 'Superior': 0,'Inferior': 0, 
                                                        'Anterior': 0, 'Posterior': 0, 
                                                        'Right': 0, 'Left': 0 })
...
def run():
    ...
    c_ptv1 = virtual_PTV(ptv1,d1,f'zzRing_{ptv1}','White')
    c_ptv2 = virtual_PTV(ptv2,d2,f'zzRing_{ptv2}', 'White')
    c_ptv3 = virtual_PTV(ptv3,d3,f'zzRing_{ptv3}', 'White')
    c_ptv4 = virtual_PTV(ptv4,d4,f'zzRing_{ptv4}', 'White')
    ...
    for s in sub_list :
        if s.source != "--None--":
            ...
            Ring(s.source,dis=s.distance,virname=s.virtual_name,color=s.color)
