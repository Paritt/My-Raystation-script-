...
def FiF(case, plan, beamset, subMedTang="3",subLatTang="4",initial_MU = 10,step_MU=2, accept_hot=1.05, PTV_name="PTV", Body_name="BODY", n=0):
	...
	while max_dose<accept_hot:
		...
		beamset.Beams[subLatTang].BeamMU = initial_MU
		beamset.SetMUAndComputeDose(ForceRecompute=True)
		max_dose = plan.TreatmentCourse.TotalDose.GetDoseAtRelativeVolumes(RoiName=f"z maxEval_{n}", RelativeVolumes=[0])/beamset.Prescription.PrimaryPrescriptionDoseReference.DoseValue
...
def Start():
	...
	for n in range(3,N+3):
		FiF(case, plan, beamset, subMedTang=str(n),subLatTang=str(n+1),initial_MU = initial_MU,step_MU=step_MU, accept_hot=accept_hot, PTV_name=PTV_name, Body_name=Body_name, n=n)
		progress_bar["value"] += N
		root.update_idletasks()
