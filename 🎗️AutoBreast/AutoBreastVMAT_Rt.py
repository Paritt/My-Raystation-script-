...
def Button_MatchRoi():
    ...
    combo_LtLung = lbMatch(Sub_root1, ListRoi, roi = lt_lung, row_n = 2)
    combo_RtLung = lbMatch(Sub_root1, ListRoi, roi = rt_lung, row_n = 3)
    combo_LtBreast = lbMatch(Sub_root1, ListRoi, roi = lt_breast, row_n = 4)
    combo_heart = lbMatch(Sub_root1, ListRoi, roi = heart, row_n = 5)
    combo_thyroid = lbMatch(Sub_root1, ListRoi, roi = thyroid, row_n = 6)
    combo_eso = lbMatch(Sub_root1, ListRoi, roi = eso, row_n = 7)
    combo_body = lbMatch(Sub_root1, ListRoi, roi = body, row_n = 8)
    combo_arm = lbMatch(Sub_root1, ListRoi, roi = arm, row_n = 9)
    ...
    def Button_Apply_sub1():
        ...
        try: 
            ...
            rt_lung = combo_RtLung
        try: 
            lt_breast = combo_LtBreast.get()
        except:
            lt_breast = combo_LtBreast
        try: 
...
lbMatchRoi.config(width=27, height =2)
ListRoi = [r.Name for r in patient.Cases[0].PatientModel.RegionsOfInterest]
ListRoiOpt = [ptv,ctv,lt_lung,rt_lung,lt_breast,heart,thyroid,eso,body,arm]
print(ListRoi)
...
def create_virtual():
    ...
    zSkinFlash.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ...
                                ResultMarginSettings={ 'Type': "Expand", 'Superior': 0,'Inferior': 0, 
                                                        ...
                                                        'Right': 0, 'Left': 0 })
    # zRtLung
    zRtLung = case.PatientModel.CreateRoi(Name="zzRtLung", Color="Blue", Type="Control",
                                        TissueName=None, RbeCellTypeName=None, RoiMaterial=None)
    zRtLung.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [rt_lung], 
                                            'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 
                                                                'Anterior': 0, 'Posterior': 0, 
                                                                'Right': 0, 'Left': 0 } }, 
                                ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [ptv], 
...
def addPlan():
    ...
    beam_set.UpdateDoseGrid(Corner={'x': dose_grid.Corner.x, 'y': -13, 'z': dose_grid.Corner.z},
                            VoxelSize={'x': dose_grid.VoxelSize.x, 'y': dose_grid.VoxelSize.y, 'z': dose_grid.VoxelSize.z},
                            NumberOfVoxels={'x': dose_grid.NrVoxels.x, 'y': 220, 'z': dose_grid.NrVoxels.z})
    #Isocenter placment
    ...
    print(f"arm: {arm}")
    Add_Beam1 = beam_set.CreateArcBeam(ArcStopGantryAngle=mid, ArcRotationDirection="CounterClockwise", 
                            BeamQualityId=PhotonE, IsocenterData= iso_data,
                            Name="1", Description=f"1.CCW{start}-{mid}", GantryAngle=start, CouchRotationAngle=0, 
    ...
    Add_Beam2 = beam_set.CreateArcBeam(ArcStopGantryAngle=stop, ArcRotationDirection="CounterClockwise", 
                            BeamQualityId=PhotonE, IsocenterData= iso_data,
                            Name="2", Description=f"2.CCW{mid}-{stop}", GantryAngle=mid, CouchRotationAngle=0, 
    ...
    Add_Beam3 = beam_set.CreateArcBeam(ArcStopGantryAngle=mid, ArcRotationDirection="Clockwise", 
                            BeamQualityId=PhotonE, IsocenterData= iso_data,
    ...
    Add_Beam4 = beam_set.CreateArcBeam(ArcStopGantryAngle=start, ArcRotationDirection="Clockwise", 
                            BeamQualityId=PhotonE, IsocenterData= iso_data,
...
def AddCostFunction():
    ...
    min_dvh(Po,"zzPTV", dose_level=int(DosePrescription*0.97), percent_volume=100, weigth=110)
    ...
    #uniform_constraint(Po, "zzPTV", stdv=2)
    ...
    max_dvh(Po, "zzRtLung", dose_level=3000, percent_volume=0, weigth=1)
    max_dvh(Po, "zzRtLung", dose_level=2000, percent_volume=5, weigth=1)
    max_dvh(Po, "zzRtLung", dose_level=1000, percent_volume=15, weigth=1)
    max_dvh(Po, "zzRtLung", dose_level=500, percent_volume=20, weigth=1)
    max_eud(Po, "zzRtLung", dose_level=2000, eud_parameter=7, weigth=1)
    max_dvh(Po, rt_lung, dose_level=2000, percent_volume=25, weigth=1)
    max_dvh(Po, rt_lung, dose_level=500, percent_volume=35, weigth=1)
    ...
    max_eud(Po, lt_breast, dose_level=500, eud_parameter=5, weigth=10)
    max_dvh(Po, lt_lung, dose_level=450, percent_volume=0, weigth=10)
    max_eud(Po, lt_lung, dose_level=500, eud_parameter=5, weigth=10)
