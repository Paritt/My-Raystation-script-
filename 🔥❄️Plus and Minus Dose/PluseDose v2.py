...
def create_contour_from_dose(dose, dose_level, name, X, contract, body,crop_body):
	...
	roi.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [f"{name}"], 
                                            'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 
                                                                'Anterior': 0, 'Posterior': 0, 
                                                                'Right': 0, 'Left': 0 } }, 
                                ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [X], 
                                            'MarginSettings': { 'Type': "Contract", 'Superior': contract, 'Inferior': contract, 
                                                                'Anterior': contract, 'Posterior': contract, 
	...
	roi.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ...
                                ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [body], 
                                            'MarginSettings': { 'Type': "Contract", 'Superior': crop_body, 'Inferior': crop_body, 
                                                                'Anterior': crop_body, 'Posterior': crop_body, 
                                                                'Right': crop_body, 'Left': crop_body } }, 
                                ResultOperation="Intersection", 
                                ResultMarginSettings={ 'Type': "Expand", 'Superior': 0,'Inferior': 0, 
                                                        'Anterior': 0, 'Posterior': 0, 
                                                        'Right': 0, 'Left': 0 })                                                    
...
def Button_Next():
	...
	crop_body = crop_body.get()
	body = body.get()
	dose = plan.TreatmentCourse.TotalDose
	create_contour_from_dose(dose, dose_level, name, X, contract, body,crop_body)
	root.destroy()
...
lbContract = Label(root, text= 'Contract (cm)')
lbContract.grid(column=0 ,row=4)
lbContract.config(width=27, height =2)
contract = StringVar()
textcontract = Entry(root, textvariable = contract)
textcontract.grid(column=1,row=4)
textcontract.config(width=27)
textcontract.insert(0, "0.1")

lbcrop_body = Label(root, text= 'Crop Body')
lbcrop_body.grid(column=0 ,row=5)
lbcrop_body.config(width=27, height =2)
crop_body = StringVar()
textcrop_body = Entry(root, textvariable = crop_body)
textcrop_body.grid(column=1,row=5)
textcrop_body.config(width=27)
textcrop_body.insert(0, "0.3")
...
buttNext.grid(column=1,row=7,padx=10,pady=25)
