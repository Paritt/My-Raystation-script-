...
def create_virtual():
    ...
    # Between_Hippo'
    zBetween_Hippo = case.PatientModel.CreateRoi(Name="zzBetween_Hippo", Color="Blue", Type="Control",
                                                    TissueName=None, RbeCellTypeName=None, RoiMaterial=None)  
    zBetween_Hippo.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ExpressionA={ 'Operation': "Union", 'SourceRoiNames': ["Hippocampus_L"], 
                                            'MarginSettings': { 'Type': "Expand", 'Superior': 0.5, 'Inferior': 0.5, 
                                                                'Anterior': 0.5, 'Posterior': 0.5, 'Right': 4, 'Left': 0.5 } }, 
                                ExpressionB={ 'Operation': "Union", 'SourceRoiNames': ["Hippocampus_R"], 
                                            'MarginSettings': { 'Type': "Expand", 'Superior': 0.5, 'Inferior': 0.5, 
                                                                'Anterior': 0.5, 'Posterior': 0.5, 'Right': 0.5, 'Left': 4 } }, 
...
def uniform_dose(po, roi_name, dose_level, weigth, beam_set_index=0):
...
def AddCostFunction():
    ...
    #############################################################################
    template = db.LoadTemplateOptimizationFunctions(templateName='Faro__Benz') 
    #############################################################################
    #############################################################################
    #############################################################################
...
def create_minus_dose(dose, dose_level, name, X):
	roi = case.PatientModel.CreateRoi(Name=f"{name}", Color="Yellow", Type="Control", TissueName=None, RbeCellTypeName=None, RoiMaterial=None)
	beamset = get_current('BeamSet')
	threshold_level = dose_level
	roi.CreateRoiGeometryFromDose(DoseDistribution = dose, ThresholdLevel = threshold_level)
...
def Button_Next():
    ...
    try:
        ...
        try:
            ...
            ############################################################
            multiple_optz(optz_number=2)
            elaps = time.time()
            elaps = elaps - start_time
            hours = elaps // 3600
            ...
            # Add MaxEUD Hippo
            ############################################################
            print("Add MaxEUD Hippo")
            max_eud(Po, hippo_l,dose_level=10.5*100, eud_parameter=20, weigth=10)
            max_eud(Po, hippo_r,dose_level=10.5*100, eud_parameter=20, weigth=10)
            multiple_optz(optz_number=2)
            elaps = time.time()
            elaps = elaps - start_time
            hours = elaps // 3600
            ...
            ############################################################
            # Add Uniform dose zzPTV_uni
            ############################################################
            print("Add Uniform dose zzPTV_uni")
            uniform_dose(Po, "zzPTV_uni", dose_level=30.8*100, weigth=200)
            multiple_optz(optz_number=2)
            elaps = time.time()
            elaps = elaps - start_time
            hours = elaps // 3600
            ...
            beamset = get_current('BeamSet')
            zDose105.CreateRoiGeometryFromDose(DoseDistribution = plan.TreatmentCourse.TotalDose, ThresholdLevel = 31.5*100)
            if ptv_boost in ListRoi:
                zDose105.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                    ...
                                    ResultMarginSettings={ 'Type': "Expand", 'Superior': 0,'Inferior': 0, 
                                                            'Anterior': 0, 'Posterior': 0, 
                                                            'Right': 0, 'Left': 0 })
            case.PatientModel.StructureSets['CT Plan'].SimplifyContours(RoiNames=["zzDose_105%"], RemoveHoles3D=False, RemoveSmallContours=True, AreaThreshold=0.05, 
                                                                        ReduceMaxNumberOfPointsInContours=False, MaxNumberOfPoints=None, CreateCopyOfRoi=False, ResolveOverlappingContours=False)                                                
            max_dose(Po, "zzDose_105%", dose_level=31*100, weigth=50)
            multiple_optz(optz_number=1)
            elaps = time.time()
            elaps = elaps - start_time
            hours = elaps // 3600
            minutes = (elaps % 3600)//60
            ...
            # Convert Plus dose
            ############################################################
            print("Convert Plus dose")
            zPlus_PTV = create_plus_dose(plan.TreatmentCourse.TotalDose, 30.1*100, f"zz+{DosePrescription/100}", X="zzPTV")
            zPlus_PTV.CreateAlgebraGeometry(Examination=examination, Algorithm="Auto", 
                                ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [f"zz+{DosePrescription/100}"],
                                            'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 
                                                                'Anterior': 0, 'Posterior': 0, 
                                                                'Right': 0, 'Left': 0 } }, 
                                ExpressionB={ 'Operation': "Union", 'SourceRoiNames': ["zzHippo_plus"], 
                                ...
                                ResultMarginSettings={ 'Type': "Expand", 'Superior': 0,'Inferior': 0, 
                                                        ...
                                                        'Right': 0, 'Left': 0 })
            '''
            case.PatientModel.StructureSets['CT Plan'].SimplifyContours(RoiNames=[f"zz+{DosePrescription/100}"], RemoveHoles3D=False, RemoveSmallContours=True, AreaThreshold=0.05, 
                                                                        ReduceMaxNumberOfPointsInContours=False, MaxNumberOfPoints=None, CreateCopyOfRoi=False, ResolveOverlappingContours=False)
            '''
            min_dvh(Po, f"zz+{DosePrescription/100}", dose_level=30.5*100, percent_volume=100, weigth=50)
            if ptv_boost in ListRoi:
            ...
            ############################################################
            # Add Max dose Brainstem, Optic chaiasm, Optic Nerve R/L
            ############################################################
            print("Add Max dose Brainstem, Optic chaiasm, Optic Nerve R/L")
            max_dose(Po, brainstem, dose_level=31*100, weigth=300)
            max_dose(Po, optic_l, dose_level=31*100, weigth=300)
            max_dose(Po, optic_r, dose_level=31*100, weigth=300)
            ############################################################
            # Convert Hot 107%
            ############################################################
            print("Convert Hot 107%")
            zDose107 = case.PatientModel.CreateRoi(Name="zzDose_107%", Color="Yellow", Type="Control", TissueName=None, RbeCellTypeName=None, RoiMaterial=None)
            beamset = get_current('BeamSet')
            zDose107.CreateRoiGeometryFromDose(DoseDistribution = plan.TreatmentCourse.TotalDose, ThresholdLevel = 31.9*100)
            case.PatientModel.StructureSets['CT Plan'].SimplifyContours(RoiNames=["zzDose_107%"], RemoveHoles3D=False, RemoveSmallContours=True, AreaThreshold=0.05, 
                                                                        ReduceMaxNumberOfPointsInContours=False, MaxNumberOfPoints=None, CreateCopyOfRoi=False, ResolveOverlappingContours=False)       
            max_dose(Po, "zzDose_107%", dose_level=31.5*100, weigth=50)  
            multiple_optz(optz_number=1)
            elaps = time.time()
            elaps = elaps - start_time
            hours = elaps // 3600
            ...
            print('6th')
            ############################################################
            # Convert Hot 105% in Between Hippo
            ############################################################
            print("Convert Hot 105% in Between Hippo")
            zDose105_inBe = create_minus_dose(plan.TreatmentCourse.TotalDose, 31.4*100, "zz-31.4inBe", X="zzBetween_Hippo")
            max_dose(Po, "zz-31.4inBe", dose_level=31*100, weigth=50)  
            multiple_optz(optz_number=1)  
