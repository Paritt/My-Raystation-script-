...
textTotalDose = Entry(root, textvariable = TotalDose_Gy_boost)
textTotalDose.grid(column=1,row=5)
textTotalDose.config(width=27)
...
combo_variableRoiPre = StringVar()
combo_valuesRoiPre = [r.Name for r in patient.Cases[0].PatientModel.RegionsOfInterest if r.Type == 'Ctv' or r.Type == 'Ptv' or r.Type == 'Gtv']
combo_valuesRoiPre.insert(0,"--None--")
comboRoiPre = ttk.Combobox(root, values= combo_valuesRoiPre, textvariable=combo_variableRoiPre)
...
def Button_MatchRoi():
    ...
    def Button_Apply_sub1():
        ...
        ptv_boost = apply(ptv_boost, combo_ptv_boost)
...
def AddCostFunction():
    ...
    if ptv_boost in ListRoi:
        min_dvh(Po, ptv_boost, dose_level=DosePrescription_boost*1.01, percent_volume=99.5, weigth=300)
        max_dvh(Po,ptv_boost, dose_level=DosePrescription_boost*1.05, percent_volume=0.1, weigth=280)
        fall_off(Po, "zzPTV", high_dose_level=DosePrescription_boost*1.05, low_dose_level=DosePrescription*1.01, distance=1.3, weigth=500)
        max_dvh(Po,"zzPTV", dose_level=DosePrescription*1.05, percent_volume=1, weigth=300)
        max_dvh(Po,"zzPTV", dose_level=DosePrescription*1.07, percent_volume=0.5, weigth=300)
        max_dose(Po, hippo_tol, dose_level=15*100, weigth=300)
        max_dvh(Po,"zzRing_boost", dose_level=DosePrescription_boost*0.95, percent_volume=0, weigth=200)
